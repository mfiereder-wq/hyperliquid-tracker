...
export function TradesTable({
  ...
  const filteredFills =
    filter === "all"
      ? displayFills
      : displayFills.filter((f) =>
          filter === "long" ? f.side === "B" : f.side === "A",
        );
  ...
  const handleExport = () => {
    exportToCSV(
      fills,
      `hyperliquid_trades_${walletAddress?.slice(0, 8) || "export"}`,
    );
  };
        ...
        <table className="trade-table">
          ...
          <tbody>
            {filteredFills.map((fill, idx) => {
              ...
              const closedPnl = fill.closedPnl
              ...
              return (
                ...
                >
                  <td
                    ...
                    title={formatTimestampFull(fill.time)}
                  >
                  ...
                  <td>
                    <span
                      className={`badge ${isBuy ? "badge-long" : "badge-short"}`}
                    >
                      {isBuy ? "BUY" : "SELL"}
                    </span>
                  </td>
                  ...
                  <td
                    className={`font-mono font-semibold ${closedPnl === null ? "text-[var(--muted)]" : closedPnl >= 0 ? "profit" : "loss"}`}
                  >
            ...
            >
              ...
              {closedPnl !== null && (
                <div className="flex justify-between items-center mt-2 pt-2 border-t border-[var(--border)]">
                  ...
                  <span
                    className={`font-mono font-semibold ${closedPnl >= 0 ? "profit" : "loss"}`}
                  >
                    {formatUSD(closedPnl)}
                  </span>
                </div>
      ...
      {fills.length > 20 && (
        <div className="p-4 border-t border-[var(--border)] text-center">
          ...
          >
            {showAll
              ? "Weniger anzeigen"
              : `Alle ${fills.length} Trades anzeigen`}
          </button>
        </div>
      )}
    </div>
