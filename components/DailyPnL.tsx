    ...
    <div className="bg-[var(--card)] border border-[var(--border)] rounded-xl p-6">
        ...
        {sortedDays.map(([date, data]) => {
          ...
          return (
            <div key={date} className="flex items-center gap-3 group">
              ...
              <div className={`w-24 text-right text-sm font-mono ${isPositive ? 'profit' : 'loss'}`}>
                {formatUSD(data.pnl)}
              </div>
      ...
      {sortedWeeks.length > 1 && (
        <div className="mt-6 pt-4 border-t border-[var(--border)]">
          <h4 className="text-sm font-medium text-[var(--muted)] mb-3">Wochenübersicht</h4>
          <div className="grid grid-cols-4 gap-2">
            {sortedWeeks.slice(0, 4).map(([week, data]) => {
              const isPositive = data.pnl >= 0;
              return (
                <div key={week} className="text-center p-2 rounded-lg bg-[var(--bg)]">
                  <div className="text-xs text-[var(--muted)] mb-1">{week}</div>
                  <div className={`text-sm font-semibold ${isPositive ? 'profit' : 'loss'}`}>
                    {formatUSD(data.pnl)}
                  </div>
                  <div className="text-xs text-[var(--muted)]">{data.trades} Trades</div>
                </div>
              );
            })}
          </div>
