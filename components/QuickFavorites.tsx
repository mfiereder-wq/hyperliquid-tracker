...
export function QuickFavorites({ onSelect, currentAddress }: QuickFavoritesProps) {
  ...
  const removeFavorite = (address: string, e: React.MouseEvent) => {
    e.stopPropagation();
    const updated = favorites.filter(f => f.address.toLowerCase() !== address.toLowerCase());
    setFavorites(updated);
    localStorage.setItem('hyperliquid-favorites', JSON.stringify(updated));
  };
    ...
    <div className="w-full">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-text-secondary">Schnellzugriff</span>
            ...
            <div className="relative">
              <div className="flex items-start justify-between mb-2">
                ...
                <button
                  ...
                  title="Entfernen"
                >
                  <svg className="w-4 h-4 text-text-muted hover:text-loss" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="flex items-center gap-3 text-xs text-text-muted">
                <span className="flex items-center gap-1">
                  ...
                  {fav.lastViewed 
                    ? getRelativeTime(fav.lastViewed)
                    : 'Nie'
                  }
                </span>
                {currentAddress?.toLowerCase() === fav.address.toLowerCase() && (
...
function getRelativeTime(timestamp: number): string {
  ...
  return new Date(timestamp).toLocaleDateString('de-DE', { month: 'short', day: 'numeric' });
}
