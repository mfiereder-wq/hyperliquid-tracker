  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('Failed to deserialize')) {
        throw new Error('Ungültige Wallet-Adresse. Bitte überprüfe die Adresse.');
      }
...
export function calculateTradeStats(fills: any[]): {
  ...
  const totalVolume = fills.reduce((sum, f) => sum + parseFloat(f.px) * parseFloat(f.sz), 0);
  ...
  const longPnl = longWithPnl.reduce((sum, f) => sum + parseFloat(f.closedPnl || '0'), 0);
  const shortPnl = shortWithPnl.reduce((sum, f) => sum + parseFloat(f.closedPnl || '0'), 0);
  
  const longWins = longWithPnl.filter(f => parseFloat(f.closedPnl || '0') > 0).length;
  const shortWins = shortWithPnl.filter(f => parseFloat(f.closedPnl || '0') > 0).length;
  ...
  return {
    ...
    longPnl,
    shortPnl,
    longWinRate: longWithPnl.length > 0 ? (longWins / longWithPnl.length) * 100 : 0,
    shortWinRate: shortWithPnl.length > 0 ? (shortWins / shortWithPnl.length) * 100 : 0,
    pnlByDay,
    pnlByWeek,
  };
}
